...
func first(connection net.Conn, username string) {
	...
	first_msg := fmt.Sprintf("Displaying Fisrt Messaage:- %v", first)
	// first_msg := []byte(fmt.Sprintf("%v", first.(interface{})))
	connection.Write([]byte(first_msg))
	fmt.Println("Displaying fisrt Functin Called", first)
}

func last(connection net.Conn, username string) {
	first := x[username].Back().Value
	first_msg := fmt.Sprintf("Displaying Last Messaage:- %v", first)
	// first_msg := []byte(fmt.Sprintf("%v", first.(interface{})))
	connection.Write([]byte(first_msg))
	fmt.Println("Displaying Last Functin Called", first)
}
